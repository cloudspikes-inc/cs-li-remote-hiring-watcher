...
def build_auth_url(client_id, redirect_uri, state, scopes):
    ...
    print(f"[DEBUG] Auth URL: {url}", file=sys.stderr)
    return url
...
def main():
    ...
    ap.add_argument(
        "--client-id", default=os.getenv("LI_CLIENT_ID"))
    ap.add_argument("--client-secret",
                    default=os.getenv("LI_CLIENT_SECRET"))
    ...
    if not args.client_id:
        sys.exit(
            "❌ Error: Missing LI_CLIENT_ID environment variable or --client-id argument")
    if not args.client_secret:
        sys.exit(
            "❌ Error: Missing LI_CLIENT_SECRET environment variable or --client-secret argument")

    print(
        f"[DEBUG] Client ID: {args.client_id[:10]}...{args.client_id[-5:]}", file=sys.stderr)
    print(
        f"[DEBUG] Redirect URI: {args.redirect_uri}", file=sys.stderr)
    ...
    server = HTTPServer(
        ("localhost", args.port), CodeCatcher)
    threading.Thread(
        target=server.handle_request, daemon=True).start()
    ...
    if not code:
        sys.exit(
            "No authorization code captured from LinkedIn redirect.")
